...
class Runner(AbstractEnvRunner):
    ...
    def __init__(self, *, env, model, ranking_buffer, nsteps, gamma, lam, batch_size, nbatch, total_timesteps):
    ...
    def run(self, update, do_buffer=False, do_sl=False, sl_num=5, lr=0.0001):
        ...
        for _ in range(self.nsteps):
            ...
            for info in infos:
                ...
                if maybeepinfo:
                    ...
                    if do_buffer:
                        self.w1, self.w2 = self.ranking_buffer.insert(np.array(self.obs_buf), np.array(self.acs_buf), maybeepinfo['r'], update*self.nbatch, self.total_timesteps, self.w1, self.w2)
                    self.obs_buf = []
                    self.acs_buf = []
                    if do_sl:
...
def learn(policy, env, ranking_buffer, args, ent_coef=0.01,
            ...
            save_interval=10000, load_path=None):
    ...
    runner = Runner(env=env, model=model, ranking_buffer=ranking_buffer, nsteps=nsteps, gamma=gamma, lam=lam, batch_size=batch_size, nbatch=nbatch,total_timesteps=args.sl_until)
    ...
    for update in range(1, nupdates+1):
        ...
        obs, returns, masks, actions, values, neglogpacs, states, epinfos = runner.run(update, do_buffer, do_sl, args.sl_num, lrnow)
