    ...
    def sample(self, batch_size):
        if self.choice=="original":
            ...
            if np.sum(self.data[:,-1])==0:
                self.p = [float(1/self.index)]*self.index
            else:
                self.p = list(self.data[:,-1]/np.sum(self.data[:,-1]))
            idx = np.random.choice(range(0,self.index), batch_size, p=self.p)
        elif self.choice=="epsilon-greedy":
            epsilon = 0.25
            if np.random.uniform(0,1)<epsilon:
                idx = np.random.choice(range(0,self.index), batch_size)
            else:
                idx = np.array(list(range(self.index-batch_size, self.index)))
