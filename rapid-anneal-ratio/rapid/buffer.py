...
class RankingBuffer(object):
    ...
    def insert(self, obs, acs, ret, cur_tmp, total_tmp):
        start_tmp = int(total_tmp*self.ratio)
        if cur_tmp>start_tmp:
            cur_w1 = self.w1*(total_tmp-cur_tmp)/(total_tmp-start_tmp)
            cur_w2 = self.w2*(total_tmp-cur_tmp)/(total_tmp-start_tmp)
        elif cur_tmp<=start_tmp:
            cur_w1 = self.w1
            cur_w2 = self.w2
        ...
        if self.data is None:
            ...
            if cur_w2 > 0:
                ...
                global_bonus = 0.0
            self.data[:,-1] = self.w0 * self.data[:,-3] + cur_w1 * self.data[:,-2] + cur_w2 * global_bonus
            self.data = self.data[self.data[:,-1].argsort()][-self.size:]
        self.index = self.data.shape[0]        
        print("current weights:", cur_w1, cur_w2)
...
def get_local_bonus_v1(obs, score_type):
    if score_type == 'discrete':
        motivation_dict = {4:2, 5:2, 7:2}
        obs = obs.reshape((obs.shape[0], -1))
        unique_obs = np.unique(obs, axis=0)
        total = obs.shape[0]
        unique = unique_obs.shape[0]
        ...
        for i in range(unique):
            for j in range(0, unique_obs.shape[1], 3):
                if unique_obs[i,j] in motivation_dict:
                    score += float(motivation_dict[unique_obs[i,j]] / total)
                else:
                    score += float(1/total)
    elif score_type == 'continious':
        obs = obs.reshape((obs.shape[0], -1))
