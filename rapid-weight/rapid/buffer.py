...
class RankingBuffer(object):
    def __init__(self, ob_space, ac_space, args):
        ...
        self.local_type = args.local_score_type
        self.w0 = args.w0
        self.w1 = args.w1
        self.w2 = args.w2
    ...
    def insert(self, obs, acs, ret):
        ...
        if cur_w1 > 0:
            if self.local_type=="original":
                local_bonus = get_local_bonus(obs, self.score_type)
            elif self.local_type=="new":
                local_bonus = get_local_bonus_new(obs, self.score_type)
        else:
            local_bonus = 0.0
...
def get_local_bonus_new(obs, score_type):
    if score_type == 'discrete':
